...
(defn add-method [multimethod dispatch-value fn]
  (.addMethod multimethod dispatch-value fn))

(defonce activation-body
  (delay
   (add-method assert-expr '=
               (fn [msg [_ a & more]]
                 `(let [a# ~a]
   ...
   (add-method report :fail
               (fn [{:keys [type expected actual diffs message] :as event}]
                 (with-test-out
